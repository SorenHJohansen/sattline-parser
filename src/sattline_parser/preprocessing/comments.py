"""SattLine comment stripping helpers.

These remove ``(* ... *)`` comments (including nested ones) from source
text before parsing, preserving a mapping from cleaned-text offsets back
to the original source for error reporting.
"""
...
def strip_sl_comments(text: str) -> str:
    ...
    - A newline ends a string if it hasn't been closed yet. Both LF and CR and
      CRLF are handled; CRLF is preserved as-is.
